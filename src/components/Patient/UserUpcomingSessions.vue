  ...
  methods: {
    async displayUpcomingSessions(user) {
      ...
      for (const s of this.upcomingArr) {
        ...
        if (10 * 60 * 1000 < sessionTime - timeNow) {
          // it is OVER 10 mins till the start of the session time
          console.log(
            "No link yet, and DOES NOT meet criteria to create  a room now. Session: ",
            sessionTime,
            ", timeNow: ",
            timeNow
          );
          cell4.innerHTML =
            "You can enter your session room up to 10 minutes before the slot timing.";
        } else {
          // (1) within 10 minutes from session start, or (2) session slot already begun, or (3) link already exists.
          console.log(
            "No link yet, but meets criteria to create a room now. Session: ",
            sessionTime,
            ", timeNow: ",
            timeNow
          );
          ...
          linkSession.innerHTML =
            "<button class='btn btn-link btn-sm text-info'>Enter Session Room Now!";
        ...
        bu.innerHTML =
          "<button class='btn btn-link btn-sm text-error'>Cancel<button>";
        bu.onclick = () => {
