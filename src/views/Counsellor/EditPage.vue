<template>
  ...
  <div id="main" class="flex-auto justify-center">
    <p class="text-3xl font-semibold">{{ this.title }}</p>
    <br />

    <p class="italic text-sm" id="author">
      Posted on {{ this.postDate }} by {{ this.author }}
    </p>
    <br />

    <div v-if="!showEdit">
      <div class="flex justify-center gap-4">
        <button @click="showEditor()" class="btn btn-secondary btn-sm">
          Edit
        </button>
        <button
          @click="deleteArticle()"
      ...
      <br />
      <div class="mx-60 text-justify" v-html="this.mainText"></div>
      <br /><br />
    </div>
    ...
    <div v-else>
      ...
      <br />
      <div class="flex justify-center gap-4">
        <button class="btn btn-outline btn-secondary" @click="showEditor()">
          Back
        </button>
  ...
  methods: {
    ...
    async edit() {
      ...
      updateDoc(docRef, {
        ...
        .then(
          alert(
            "Your article has been successfully edited. Please wait while we refresh your page."
          ),
    ...
    async deleteArticle() {
      if (confirm("You are about to delete '" + this.title + "'. Confirm?")) {
        ...
        await updateDoc(doc(db, "Counsellors", String(this.fbuser)), {
          my_articles: arrayRemove(this.title),
        });
